-- Canonical AI judgment persistence for arena debates.
-- One judgment per room is stored and then materialized into each participant's debate_results row.

create table if not exists public.arena_judgements (
...
alter table public.arena_judgements enable row level security;
...
begin
  ...
  v_feedback := coalesce(v_judgement->>'feedback', 'AI judge feedback unavailable.');
  v_quote := coalesce(v_judgement->>'quote', 'A clear claim is strongest when it is proven.');
  ...
  v_clarity := least(greatest(coalesce((v_judgement->>'clarity_score')::numeric, 3.5), 0), 5);
  v_evidence := least(greatest(coalesce((v_judgement->>'evidence_score')::numeric, 3.5), 0), 5);
  ...
  loop
    ...
    participant_payload := jsonb_build_object(
      ...
      'feedback', concat(v_feedback, ' Decision rationale: ', v_rationale),
      'quote', v_quote,
      'scores', jsonb_build_object('clarity', v_clarity, 'evidence', v_evidence),
